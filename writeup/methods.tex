...
At this single seed, composite modeling wins by \textbf{+22.0\% final
hypervolume} -- the largest margin of any composite-vs-direct comparison
in this work (vs.\ $\sim$0.3\% on the synthetic DTLZ2-family problems in
Sections~\ref{sec:composite} and~\ref{sec:kronecker-ablation}). Checking
\texttt{tr\_restarts} rules out a restart-driven explanation (zero
restarts fired in either run): the improvement traces to one 10-point
batch (between the 170th and 180th evaluation) whose candidates land in
a substantially better region of raw trajectory space, producing a
21.3\% single-step jump in true hypervolume that a smooth, restart-free
optimization trajectory then holds.

\paragraph{Correction after a proper 5-seed run: the single-seed result
above was noise, not a real effect.} The single-seed table was, for a
long time, the only evidence for this benchmark -- unlike every other
composite benchmark in this project, Penicillin's direct-vs-composite
comparison was never run at $n=5$ to check whether $+22.0\%$ held up. It
does not. Run properly (\texttt{cluster/submit\_penicillin\_5seed.sh},
5 seeds each label, same $d{=}7$, $M{=}3$, $K{=}5$ checkpoint
configuration as above): mean final hypervolume is $374{,}540.9$ (std
$31{,}125.7$) for \texttt{composite\_penicillin} versus $363{,}356.2$
(std $16{,}568.4$) for \texttt{morbo} -- a $+3.0\%$ mean delta, win-rate
only $3/5$, and \textbf{not} statistically significant (paired $t$-test
$p=0.333$, 95\% CI on the paired \% delta $[-4.6\%,+10.7\%]$, cleanly
straddling zero). AUC tells the same story: $-0.55\%$ mean delta,
win-rate $2/5$, $p=0.759$. The seed-0 run that produced the original
$+22.0\%$ table was simply the luckiest of the five -- composite
modeling's own std ($31{,}125.7$) is nearly twice direct modeling's
($16{,}568.4$), so a single seed landing on one side or the other of a
noisy, high-variance comparison was never strong evidence either way.
This retracts the "largest margin of any composite comparison in this
work" claim above: Penicillin is, at proper sample size, a null result
like SnAr and GRI-Mech v1 (\S\ref{sec:gri-mech}'s own results
discussion already anticipated this alongside SnAr, before this section
had multi-seed data of its own to confirm it), not the standout case it
appeared to be at $n=1$. The lesson generalizes: every headline number
in this project reported at $n=1$ deserves the same skepticism until
run at the same $n=5$ standard as the rest of the composite ablation.
...
Section~\ref{sec:penicillin}, confirmed a null result only after a
proper 5-seed run -- see that section's own correction paragraph) --
none of which showed a statistically significant HV difference, for a
mix of reasons: SnAr's generous eval budget relative to problem
...
paragraph and the SnAr plot analysis), while Penicillin's null result
instead reflects genuinely high seed-to-seed variance swamping a small
or nonexistent effect. Here that same generic budget
...
\subsection{Implemented: composite RCM46 Optimal Power Flow}
\label{sec:rcm46-implemented}

RCM46 is the same real IEEE 14-bus test system as RCM40
(Section~\ref{sec:rcm40-implemented}) -- identical admittance data,
identical 34-dimensional design space -- but case 46 of the CEC2021
RWCMOP suite \citep{kumar2021rwcmop} computes \emph{four} objectives
(fuel cost, active-power loss, reactive-power loss, and voltage
deviation) rather than two, testing whether more, and more-decoupled,
objectives on the same genuinely bus-decoupled raw response amplify
RCM40's modest composite advantage. Implemented in
\texttt{morbo/\allowbreak problems/\allowbreak composite\_rcm46.py}, the raw response is
the 45-dim per-bus injection/generation/voltage vector; the reduction is
the four objectives, each a decoupled function of it. All constants were
extracted directly from \texttt{CEC2021\_func.m}'s \texttt{case 46}, the
same extract-don't-retype discipline as RCM40 (and RCM46's $f_3$ term
uses the matched-bus $\operatorname{Im}(V_1 \bar I_1)$, unlike RCM40's
apparent $I_2$ index slip, so RCM40's version is most likely a typo in
the original suite -- documented, not silently reconciled).

\paragraph{Results.} This is the strongest, cleanest real-world
composite win in this work. Across 5 seeds (300 evaluations, no SLURM
time limit, \texttt{cluster/submit\_rcm46\_2x1.sh}), \texttt{composite\_rcm46}
beats \texttt{morbo} on \emph{final} hypervolume by $+4.5\%$ (win-rate
$5/5$, paired $t$-test $p=0.013$, 95\% CI $[+1.6\%, +7.5\%]$), and on
AUC by $+3.5\%$ ($4/5$, $p=0.031$) -- significant on both the
final-value and sample-efficiency measures, unlike RCM40 (sample
efficiency only). That the effect grows from RCM40's modest, close-by-
the-end margin to RCM46's decisive one, on the identical physical system
with only the objective count changed, is direct evidence for this
work's central claim: composite modeling helps most when the reduction
exposes genuinely decoupled structure, and four decoupled objectives
expose more of it than two.

\subsection{Cross-validation on an independent multi-method pipeline}
\label{sec:tau-crossval}

Every result above uses this project's own MORBO engine. To check that
the composite advantage is not an artifact of that one implementation,
the same benchmarks were run through a collaborator's independent
multi-method pipeline (\texttt{tau315/composite-mobo}), which drives four
solver families -- standard qEHVI (\texttt{standard\_mobo}), a Chebyshev
scalarization (\texttt{chebyshev\_bo}), a spherical-Chebyshev
scalarization for high dimension (\texttt{spherical\_chebyshev\_bo}), and
a batched trust-region method (\texttt{batched\_morbo}) -- each in a
direct and a composite variant, at $n=17$--$20$ seeds. Our RCM40, RCM46,
and Penicillin benchmarks were ported into that pipeline's own
\texttt{BenchmarkProblem} interface (verified bit-identical to our
reference implementations on shared inputs) and run there.

\paragraph{The power-flow wins replicate.} On \texttt{batched\_morbo},
composite modeling beats direct by $+0.79\%$ on RCM40 ($18/20$,
$p=0.0001$) and $+4.63\%$ on RCM46 ($17/17$, $p<0.0001$) -- the same
sign, and for RCM46 nearly the same magnitude, as our own pipeline's
$+4.5\%$. A win this size surviving a completely separate solver
codebase is much stronger evidence than either run alone.

\paragraph{Penicillin reverses under more seeds.} Our own 5-seed
Penicillin run (Section~\ref{sec:penicillin}) showed a non-significant
$+3.0\%$ ($p=0.333$); at $n=20$ through this pipeline, composite modeling
is a significant \emph{loss} on \texttt{batched\_morbo} ($-2.53\%$,
$p=0.019$) and null-to-negative on the scalarization solvers. The
apparent gain was small-sample noise, exactly the failure a
five-seed read-out is prone to -- and the corrected conclusion (Penicillin
is not a composite-friendly benchmark) is consistent with its raw
response being a checkpointed trajectory of one smooth fermentation
process, the same structure that fails for GRI-Mech v1 and OC20.

\paragraph{Method dependence is real.} The composite advantage is not
purely a property of the benchmark. On CORT TG119 (a 418-dimensional
radiotherapy problem in the collaborator's own set), composite modeling
\emph{loses} $-1.98\%$ on \texttt{batched\_morbo} ($p=0.006$) but
\emph{wins} $+1.67\%$ on \texttt{spherical\_chebyshev\_bo} ($p=0.0004$)
-- opposite signs, both significant, same benchmark. Whatever composite
modeling does for the surrogate, the acquisition/scalarization it feeds
into can turn that into a gain or a loss, a caveat any single-solver
study (including our own main results) cannot see.
