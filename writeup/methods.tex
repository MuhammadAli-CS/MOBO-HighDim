...
The design variables and their physical bounds are
\begin{align}
\tau \in [0.5, 2], \quad e \in [1.0, 5.0], \quad C_0 \in [0.1, 0.5],
\quad T \in [30, 120].
\end{align}

The benchmark integrates a five-species kinetic model
(\S\ref{sec:snar}). Let $C_p$ denote the outlet concentration of the
desired product, $q$ the total flow rate, $V$ the reactor volume, and
$M_p$ the product molecular weight. Space-time yield is
\begin{align}
\mathrm{STY} = \frac{6\times10^4\, M_p\, C_p\, q}{1000\, V}.
\end{align}
The E-factor is the mass of solvent and non-product species relative to
product mass:
\begin{align}
E = \frac{q\,\rho_{\mathrm{eth}} + 10^{-3} q \sum_{j \ne p} M_j C_j}{10^{-3} M_p C_p q}.
\end{align}
We maximize
\begin{align}
f_1 = \mathrm{STY}
...
f_2 = -E.
\end{align}
The composite raw response is the shared 6-dim vector $g(x) =
(C_{\mathrm{dfnb}}, C_{\mathrm{pldn}}, C_p, C_{\mathrm{regio}}, C_{\mathrm{bis}}, q)$
-- both $f_1$ and $f_2$ are computed from this one decomposition, rather
than each objective exposing its own separate raw response (see
\S\ref{sec:snar} for why).

\subsection{GRI-Mech 3.0 rate-constant calibration}

The design point is $x = (x_1, \ldots, x_{64}) \in [-1,1]^{64}$, one
log$_2$-scale rate multiplier per selected reaction $r_1, \ldots, r_{64}$:
\begin{align}
k_{r_i}(x_i) = 2^{x_i}\, k_{r_i}^{(0)}, \qquad i = 1, \ldots, 64.
\end{align}
For each of 6 fixed conditions $c$ (initial temperature $T_{0,c}$,
horizon $t_{\max,c}$), the raw response is a 4-point checkpointed
temperature trajectory:
\begin{align}
g_c(x) = \big(T_c(x; t_{c,1}), \ldots, T_c(x; t_{c,4})\big),
\qquad t_{c,j} = \tfrac{j}{4}\, t_{\max,c}.
\end{align}
Ignition delay $\tau_c(x)$ is recovered by linear interpolation of
$g_c(x)$ to the crossing $T_{0,c}+400$ (\S\ref{sec:gri-mech}). Writing
...
targets, we maximize
\begin{align}
f_{\mathrm{low}} = -\!\!\sum_{c \,\in\, \mathcal{C}_{\mathrm{low}}} \Big(\log \tfrac{\tau_c(x)}{\tau_c^{\mathrm{target}}}\Big)^{\!2}
\end{align}
and
\begin{align}
f_{\mathrm{high}} = -\!\!\sum_{c \,\in\, \mathcal{C}_{\mathrm{high}}} \Big(\log \tfrac{\tau_c(x)}{\tau_c^{\mathrm{target}}}\Big)^{\!2}.
\end{align}
