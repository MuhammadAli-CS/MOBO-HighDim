...
\end{align}
where $g: \mathcal{X} \to \mathbb{R}^k$ is the \emph{raw (composite)
response} -- the real simulator's intermediate physical state, run once
per evaluation of $x$ -- and $L: \mathbb{R}^k \to \mathbb{R}^M$ is the
\emph{known reduction}, a fixed, closed-form (not learned, not
re-simulated) formula mapping that raw state to the $M$ final
objectives. The \textbf{direct} evalfn (\texttt{SnAr},
\texttt{GriMechCalib}) evaluates $x \mapsto f(x)$ end to end and lets
the GP model $f(x) \in \mathbb{R}^M$ directly; the \textbf{composite}
evalfn (\texttt{CompositeSnAr}, \texttt{CompositeGriMechCalib}) instead
lets the GP model the intermediate $g(x) \in \mathbb{R}^k$, applying the
same fixed $L$ afterward to reconstruct $f(x)$ -- so both evalfns are
computing the identical function $x \mapsto f(x)$, differing only in
what the surrogate model is asked to predict.
...
\paragraph{Raw response $g(x)$.} A five-species kinetic ODE
(\S\ref{sec:snar}) integrated over $[0,\tau]$ from $x$ gives the final
concentrations plus the flow rate $q = 5\,\mathrm{mL}/\tau$ implied by
$\tau$:
\begin{align}
...
\paragraph{Known reduction $L(g)$ and objectives $f(x) = L(g(x))$.}
Writing $C_p, q$ as the relevant components of $g(x)$, $V$ the reactor
volume, and $M_p$ the product molecular weight, space-time yield is
\begin{align}
...
\paragraph{Raw response $g(x)$.} For each of 6 fixed conditions $c$
(initial temperature $T_{0,c}$, horizon $t_{\max,c}$), a 0D
constant-pressure ignition run under rate constants $k(x)$ gives a
4-point checkpointed temperature trajectory:
\begin{align}
g_c(x) = \big(T_c(x; t_{c,1}), \ldots, T_c(x; t_{c,4})\big),
\qquad t_{c,j} = \tfrac{j}{4}\, t_{\max,c},
\end{align}
and $g(x) = (g_1(x), \ldots, g_6(x)) \in \mathbb{R}^{24}$.

\paragraph{Known reduction $L(g)$ and objectives $f(x) = L(g(x))$.}
Ignition delay $\tau_c(x)$ is recovered from $g_c(x)$ by linear
interpolation to the crossing $T_{0,c}+400$ (\S\ref{sec:gri-mech}).
Writing $\mathcal{C}_{\mathrm{low}} = \{1000,1050,1100\}$~K,
