...
OC20/OC22 catalysis \citep{chanussot2021oc20,tran2022oc22} & high (atom coords.) & 2 (energy, stability) & Fixed dataset; live proxy via fairchem (\S\ref{sec:oc20}) \\
...
RCM40 optimal power flow \citep{kumar2021rwcmop,biswas2020opf} & 34 & 2 (active/reactive loss) & Closed-form, verified vs.\ source (\S\ref{sec:rcm40}) \\
...
LensAI / Optiland \citep{kramer2020lensai,kramer2020optiland} & 5--20 & 2 (spot size, aberration) & Composite but cheap (ray tracing) \\
...
LensAI / Optiland \citep{kramer2020lensai,kramer2020optiland} & 5--20 & yes (ray-traced fields) & no (ray tracing is cheap) \\
...
practical middle ground, not yet tried by us: \texttt{facebookresearch/fairchem}
(formerly \texttt{ocp}) ships pretrained GemNet-OC graph-neural-network
potentials trained on OC20/OC22, runnable for fast inference as an
approximate but genuinely open, live-queryable proxy for the true DFT
relaxation -- at the cost of the evaluation then being fast ML inference
rather than genuinely expensive DFT, the same expense trade-off already
flagged for the tabular NAS/HPO benchmarks below. (One more citation
correction, same pattern as EvoXBench's link and Optiland's author
above: a follow-up survey pass attributed OC20 to ``Montoya et al.\ 2021,
...
Honest caveat, matching this section's own standard: unlike PyBaMM
(Section~\ref{sec:pybamm}), IDAES has not yet been installed and run by
us -- the flowsheet structure above is read from IDAES's own
documentation and tutorials, not measured directly, so treat the exact
unit count and solve time as approximate until verified the same way.
...
\citep{kramer2020lensai,kramer2020optiland} is composite but cheap (ray
...
OC20/OC22 catalysis \citep{chanussot2021oc20,tran2022oc22} & high (atom coords.) & 2 (energy, stability) & Fixed dataset; live proxy via fairchem (\S\ref{sec:oc20}) \\
IDAES process flowsheets \citep{lee2021idaes} & 5--50 & 2 (yield, cost) & Real \& open; not yet run ourselves (\S\ref{sec:idaes}) \\
Summit SnAr reactor \citep{felton2021summit} & 4 & 2 (space-time yield, E-factor) & Real, composite via outlet species; not yet run ourselves \\
...
None of the three newly-added candidates (Summit, Cantera, BioSTEAM) has
been installed and run by us yet -- same honest-caveat standard as
IDAES (\S\ref{sec:idaes}) and OpenTO (\S\ref{sec:topodiff-selto}) above.
...
\bibitem{eggensperger2021hpobench}
...
\bibitem{kramer2020lensai}
H. Kramer.
\newblock LensAI: Integrated Machine and Deep Learning for Optical Design.
\newblock GitHub repository; \url{https://github.com/HarrisonKramer/LensAI}.

\bibitem{kramer2020optiland}
H. Kramer.
\newblock Optiland: optical design, optimization, and analysis in Python.
\newblock GitHub repository; \url{https://github.com/HarrisonKramer/optiland}.
...
\bibitem{andreassen2011topopt88}
...
\bibitem{cortespena2020biosteam}
...
\bibitem{rajaonson2025chemixhub}
