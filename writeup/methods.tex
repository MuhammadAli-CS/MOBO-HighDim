...
\paragraph{Targets.} Rather than depend on historical experimental
shock-tube data (as GRI-Mech's own original calibration did), the six
target ignition delays are generated by applying this benchmark's own
checkpoint-based reduction, once, to the \emph{unmodified} as-shipped
GRI-Mech 3.0 mechanism (all 64 multipliers $=1$) -- a standard synthetic
ground-truth-recovery construction: the real simulator, real mechanism,
and real sensitivity-based reaction selection are all genuine; only the
specific numeric targets being matched come from the same real mechanism
rather than from a redistributed experimental dataset. This was verified
directly: evaluating the unmodified mechanism against its own targets
gives objective values of $(-2.7\times10^{-12}, -8.3\times10^{-8})$,
i.e.\ zero error up to floating-point precision, confirming the
reduction is deterministic and self-consistent between target generation
and evaluation.
...
every condition's ignition delay, e.g.\ the 1000~K condition's target
$1.126$~s becomes roughly half that) and by a factor of 0.5 (roughly doubles
...
\texttt{morbo.run\_one\_replication.\allowbreak run\_one\_replication} for both
the \texttt{morbo} label (direct 2-objective modeling; toy-scale 10-eval
smoke test, wall time XXs) and \texttt{composite\_gri\_mech}
(composite-GP modeling of the 24-dim raw response; wall time XXs) --
XXX.
...
no SLURM time limit (composite mode's 48-output \texttt{ModelListGP} fit
