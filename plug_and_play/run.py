...
_EVALFN_MAP = {
    ...
    # "CompositeDTLZ2" is this repo's own composite-modeling DTLZ2 --
    # morbo/problems/composite_dtlz2.py hardcodes num_objectives=2 (its
    # reduction formula is only implemented for M=2), so this benchmark
    # only actually runs through the real engine at M=2, even though
    # benchmarks.py's "composite_dtlz2" itself supports any M (via the
    # separate, standalone problems/composite_dtlz2_general.py). Passing
    # num_objectives != 2 here surfaces morbo's own ValueError rather than
    # silently working around it -- an honest limitation of the real
    # engine, not something to paper over.
    "composite_dtlz2": "CompositeDTLZ2",
